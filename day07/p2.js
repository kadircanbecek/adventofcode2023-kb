...
sorting_order  = "J23456789TQKA" //"AKQJT98765432"
data = data.map(value=>{
    ...
    cards_grouped = cards.reduce((prevItem, currentItem) => ({
      ...
      {});
    jokeramount = cards_grouped["0"]
    
    cards_grouped = Object.values(cards_grouped).sort((a,b)=> b.length-a.length || b[0]-a[0])
    if (hand != 'JJJJJ' && jokeramount){
        const joker_index = cards_grouped.indexOf(jokeramount);    
        if (joker_index > -1) { // only splice array when item is found
            cards_grouped.splice(joker_index, 1); // 2nd parameter means remove one item only
        }
        cards_grouped = [cards_grouped[0].concat(Array(jokeramount.length).fill(cards_grouped[0][0]))].concat(cards_grouped.slice(1));
